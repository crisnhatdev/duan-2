...
if ($type) {
    switch ($type) {
        case 'comments':
            $cmt = $crValid->valid_value_insert($_POST['arrData'][0]['value']); //nội dung đánh giá

            $sao = $_POST['arrData'][1]['value'][0]; //lấy giá trị index = 0 trong mảng arrDat và string đầu tiên (i = 0)

            $data = explode(',', $_POST['arrData'][2]['value']); //mảng chứa 3 giá trị (bảng db, cột db, mã sp hoặc mã bv)
            $bang = $data[0];
            $tenCot = $data[1];
            $ma = $data[2];
...
//            $matk = $_SESSION['user']['idaccount'];// mã tk đánh giá
            ...
            if ($crValid->valid_text($cmt) && $cmt != '') {
                $crComm->upCmt($bang, $tenCot, $cmt, $sao, $ngaydang, $matk, $ma); //up cmt mới lên database

                $cmtList = $crComm->getCmt($bang, $tenCot, $ma); //lấy tổng số cmt ra để tính tổng điểm
                $total = 0;
                foreach ($cmtList as $cmt) {
                    $total += $cmt['sao'];
                }
                $total = $total / count($cmtList);

                $newCmt = $crComm->getCmt($bang, $tenCot, $ma, 1)[0]; // lấy cmt theo mã sp
                ...
                echo json_encode(array($output, $total, count($cmtList)));
            }
            break;
        case 'pagination-comment':
            $data = explode(',', $_POST['arrData']); //mảng chứa 3 giá trị (bảng db, cột db, mã sp hoặc mã bv)
            ...
            $limitCmts = $crComm->paginationCmts($bang, $tenCot, 3, $page, $ma); //tên bảng, tên cột, cmt giới hạn, trang, mã bv || mã sp


            $output = '';
            foreach ($limitCmts as $cmt) {
                $star = ''; // hiện thị số sao người dùng đã đánh giá
                for ($i = 0; $i < $cmt['sao']; $i++) {
                    $star .= '<i class = "fa fa-star"></i> ';
                }
